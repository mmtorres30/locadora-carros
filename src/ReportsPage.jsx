...
export default function ReportsPage() {
  ...
  const porDia = useMemo(() => {
    ...
    filtradas.forEach((r) => {
      ...
      if (!map.has(dia)) map.set(dia, { dia, retiradas: 0, devolucoes: 0 });
      map.get(dia).retiradas += 1;
      if (r.status === "fechada" && r.devolucao?.data) {
        const key = r.devolucao.data;
        if (!map.has(key)) map.set(key, { dia: key, retiradas: 0, devolucoes: 0 });
  ...
  const porPlaca = useMemo(() => {
    ...
    filtradas.forEach((r) => {
      ...
      if (!map.has(placa)) map.set(placa, { placa, modelo: r.veiculo?.modelo || "-", total: 0, abertas: 0, fechadas: 0, diasTotal: 0 });
  ...
  function exportarExcel() {
    ...
    const wsDia = XLSX.utils.json_to_sheet(porDia.map((d) => ({
      Data: d.dia, Retiradas: d.retiradas, Devoluções: d.devolucoes,
    })));
    ...
    const wsPlaca = XLSX.utils.json_to_sheet(porPlaca.map((p) => ({
      ...
      "Em andamento": p.abertas, Concluídas: p.fechadas, "Dias alugado (total)": p.diasTotal,
    })));
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="crs-section">
            ...
            <div className="crs-section-body" style={{ padding: 0 }}>
              <table className="crs-table">
                <thead><tr><th>Data</th><th>Retiradas</th><th>Devoluções</th></tr></thead>
                <tbody>
                  {porDia.length === 0 && <tr><td colSpan={3} style={{ textAlign: "center", padding: 20, color: "#8a8578" }}>Sem dados no período.</td></tr>}
                  {porDia.map((d) => (
                    <tr key={d.dia}><td className="crs-mono">{d.dia}</td><td>{d.retiradas}</td><td>{d.devolucoes}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
          ...
          <div className="crs-section">
            ...
            <div className="crs-section-body" style={{ padding: 0 }}>
              <table className="crs-table">
                <thead><tr><th>Placa</th><th>Modelo</th><th>Total</th><th>Em andamento</th><th>Concluídas</th><th>Dias alugado</th></tr></thead>
                <tbody>
                  {porPlaca.length === 0 && <tr><td colSpan={6} style={{ textAlign: "center", padding: 20, color: "#8a8578" }}>Sem dados no período.</td></tr>}
                  {porPlaca.map((p) => (
                    <tr key={p.placa}>
                      <td className="crs-mono">{p.placa}</td><td>{p.modelo}</td><td>{p.total}</td>
                      <td>{p.abertas}</td><td>{p.fechadas}</td><td>{p.diasTotal}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
